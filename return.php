...
<h1 style="color:white; text-decoration:underline;">LIBRARY  MANAGEMENT  SYSTEM</h1><br><br><br>
...
if($abc==1)
  {
	  $dlt="DELETE FROM `issuebook` WHERE `book_no`=$book_no";
	  $delete=mysqli_query($con,"$dlt");
	  if($delete==1)
	  {
		  $upadate="UPDATE `book_details` SET `available_quantity`='availabilty' WHERE `book_no`=$book_no";
		  $res=mysqli_query($con,"$upadate");
		  if($res)
		{
			echo"delete";
	    }
